...
def encode_image(image_path, vae, device, target_size=(256, 256)):
    """
    주어진 경로의 이미지를 target_size (height, width)로 리사이즈한 후 VAE를 통해 latent로 인코딩합니다.
    """
    if image_path is None:
        logger.error("Image path is None. Cannot encode image.")
        return None
    try:
        image = Image.open(image_path).convert("L")  # 그레이스케일로 로드
    ...
    preprocess = transforms.Compose([
        transforms.Resize(target_size),  # target_size로 리사이즈 (예: (320, 480))
        transforms.ToTensor(),
...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        self.clip_text_encoder = BertModel.from_pretrained(
            str(text_encoder_path), False, revision=None, ignore_mismatched_sizes=True
        ).to(self.device)
    ...
    def predict(
        ...
        # ========================================================================
        # Arguments: image_path processing - encode to latents
        # ========================================================================
        if image_path is not None:
            logger.info(f"Encoding image from path: {image_path}")
            target_size = tuple(self.args.image_size)
            image_latents = encode_image(image_path, self.vae, self.device, target_size=target_size)
