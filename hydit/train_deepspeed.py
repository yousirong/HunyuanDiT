...
from hydit.modules.text_encoder import T5Embedder
...
from hydit.best_checkpoint_tracker import BestCheckpointTracker
from hydit.evaluation_sampling import run_distributed_evaluation
...
def sample_images_multi_gpu(
    ...
    rank, world_size, args, logger, train_steps, dataset=None, loader=None
):
    """
    A more robust function to sample images during training without causing errors.
    It uses prompts from the Waymo CSV file.
    """
    model.eval()
    try:
        # Import posemb functions here to avoid circular imports
        from hydit.modules.posemb_layers import get_2d_rotary_pos_embed
        import pandas as pd
        import os

        # Load Waymo CSV file
        csv_path = "/mnt/ssd/HunyuanDiT/dataset/waymo/csvfile/image_text.csv"
        if os.path.exists(csv_path):
            df = pd.read_csv(csv_path)
            # Select a prompt based on rank to ensure different prompts per GPU
            sample_idx = (rank + train_steps // 1000) % len(df)  # Vary with training steps
            prompt = df.iloc[sample_idx]['text_en']
            reference_image_path = df.iloc[sample_idx]['image_path']
            logger.info(f"GPU {rank}: Generating sample with Waymo prompt {sample_idx}: '{prompt[:50]}...'")
        else:
            # Fallback to fixed prompts if CSV not found
            prompts = [
            ...
            logger.info(f"GPU {rank}: Generating sample with fallback prompt: '{prompt}'")
        ...
        with torch.no_grad():
            encoder_hidden_states = text_encoder(text_inputs.input_ids, attention_mask=text_inputs.attention_mask)[0]
        ...
        noise = torch.randn(shape, device=rank, dtype=torch.float16 if args.use_fp16 else torch.float32)

        # Prepare positional embeddings
        from hydit.modules.posemb_layers import calc_sizes
        
        patch_size = getattr(model, 'patch_size', 2)
        if hasattr(model, 'module'):
            patch_size = getattr(model.module, 'patch_size', 2)
        
        th = image_size // 8 // patch_size
        tw = image_size // 8 // patch_size
        ...
        # Use calc_sizes to get the correct parameters for get_2d_rotary_pos_embed
        rope_img = getattr(args, 'rope_img', 'base512')
        sub_args = calc_sizes(rope_img, patch_size, th, tw)
        
        # Debug output
        logger.info(f"GPU {rank}: Model config - model: {args.model}, hidden_size: {hidden_size}, num_heads: {num_heads}, head_dim: {head_dim}")
        logger.info(f"GPU {rank}: Image size: {image_size}, th: {th}, tw: {tw}, patch_size: {patch_size}")
        logger.info(f"GPU {rank}: rope_img: {rope_img}, sub_args: {sub_args}")
        cos_cis_img, sin_cis_img = get_2d_rotary_pos_embed(head_dim, *sub_args, use_real=True)
        cos_cis_img = cos_cis_img.to(rank)
        sin_cis_img = sin_cis_img.to(rank)
        
        # Debug output for positional embeddings
        logger.info(f"GPU {rank}: Positional embedding shapes - cos: {cos_cis_img.shape}, sin: {sin_cis_img.shape}")
        ...
        step_dir = os.path.join(sample_dir, f"step_{train_steps:07d}")
        ...
        sample_path = os.path.join(step_dir, f"gpu_{rank}_sample.png")
        pil_image.save(sample_path)

        prompt_path = os.path.join(step_dir, f"gpu_{rank}_prompt.txt")
        with open(prompt_path, 'w') as f:
            f.write(prompt)
            if 'reference_image_path' in locals() and reference_image_path:
                f.write(f"\n\nReference image: {reference_image_path}")
            
        logger.info(f"GPU {rank}: Saved sample to {sample_path}")

        if rank == 0:
            logger.info(f"Saved samples for step {train_steps} from all {world_size} GPUs to {step_dir}")

    except Exception as e:
        logger.error(f"An error occurred during sampling on GPU {rank}: {e}")
        import traceback
...
def save_checkpoint(
    ...
    world_size=None, diffusion=None, vae=None, text_encoder=None, tokenizer=None,
    text_encoder_t5=None, tokenizer_t5=None, dataset=None, loader=None, best_tracker=None
):
    ...
    # Performance evaluation and best checkpoint tracking
    if (saved and all([diffusion, vae, text_encoder, tokenizer, text_encoder_t5, tokenizer_t5])
        and world_size is not None):
        try:
            # Generate samples for visual inspection
            logger.info(f"Generating samples from all {world_size} GPUs at step {train_steps}")
            sample_images_multi_gpu(
                model, diffusion, vae, text_encoder, tokenizer, text_encoder_t5, tokenizer_t5,
                rank, world_size, args, logger, train_steps, dataset, loader
            )
            
            # Evaluation disabled to avoid NCCL timeout issues
            # Sampling will continue to work normally for visual inspection
                
        except Exception as e:
            logger.error(f"Failed to generate samples or evaluate: {e}")
            import traceback
            traceback.print_exc()
...
def main(args):
    ...
    if rank == 0:
        ...
        logger.info(f"TensorBoard logs will be saved to: {os.path.join(experiment_dir, 'tensorboard_logs')}")

    # Best Checkpoint Tracker (only on rank 0)
    best_tracker = None
    if rank == 0:
        best_tracker = BestCheckpointTracker(
            checkpoint_dir=checkpoint_dir,
            metric_name='composite_score',  # 종합 점수 기준
            higher_is_better=True,
            save_top_k=3  # 상위 3개 체크포인트 유지
        )
        logger.info(f"Best checkpoint tracker initialized")

    # Log all the arguments
    logger.info(sys.argv)
    logger.info(str(args))
    # Save to a json file (only rank 0 to avoid race condition)
    if rank == 0:
        args_dict = vars(args)
    ...
    embedder_t5 = T5Embedder(
        t5_path, torch_dtype=T5_ENCODER["torch_dtype"], max_length=args.text_len_t5
    )
    ...
    while epoch < args.epochs:
        ...
        for batch in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                samples_per_sec = steps_per_sec * batch_size * world_size
                ...
                if writer is not None:
                    ...
                    writer.add_scalar('Train/LearningRate', current_lr, train_steps)
                    writer.add_scalar('Train/StepsPerSec', steps_per_sec, train_steps)
                    writer.add_scalar('Train/SamplesPerSec', samples_per_sec, train_steps)
                    if args.grad_accu_steps > 1:
                        writer.add_scalar('Train/UpdateStep', train_steps // args.grad_accu_steps, train_steps)
        ...
        # Log epoch completion to TensorBoard
        if writer is not None:
    ...
    # Export best checkpoint results
    if best_tracker is not None and rank == 0:
        logger.info("\n" + "="*50)
        logger.info("TRAINING COMPLETED - BEST CHECKPOINT SUMMARY")
        logger.info("="*50)
        best_tracker.print_summary()
        best_tracker.export_results()
        logger.info(f"Best model available as: {checkpoint_dir}/best.pt")
        logger.info("="*50 + "\n")
