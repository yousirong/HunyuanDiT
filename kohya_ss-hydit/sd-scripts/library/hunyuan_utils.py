...
from .hunyuan_models import T5Embedder, HunYuanDiT, BertModel, DiT_g_2
...
def load_model(
    ...
    mt5_embedder = (
        T5Embedder(os.path.join(model_path, "T5"), torch_dtype=dtype, max_length=256)
        .to(device)
        .to(dtype)
    )
...
if __name__ == "__main__":
    ...
    mt5_embedder = T5Embedder(
        "./model/T5", torch_dtype=torch.float16, max_length=256
    ).cuda()
